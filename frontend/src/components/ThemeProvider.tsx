...
export default function ThemeProvider({ children }: { children: React.ReactNode }) {
    const [settings, setSettings] = useState<Settings>({
        ...
        heroTranslations: (SITE_SETTINGS as any).general?.heroTranslations ?? { en: { title: 'Your Ride Should Match the Road!', subtitle: 'Experience elite performance and unmatched prestige.' } },
        heroImageUrl: (SITE_SETTINGS as any).general?.heroImageUrl ?? 'site-1777369870612-erik-mclean-QYRdVxPeFqc-unsplash.jpg',
        smtpHost: (SITE_SETTINGS as any).smtp?.host ?? 'smtp.gmail.com',
        smtpPort: (SITE_SETTINGS as any).smtp?.port ?? '587',
        smtpUser: (SITE_SETTINGS as any).smtp?.user ?? 'sangvish21@gmail.com',
        smtpPassword: (SITE_SETTINGS as any).smtp?.password ?? '',
        smtpFrom: (SITE_SETTINGS as any).smtp?.from ?? 'sangvish21@gmail.com',
        ...
        defaultDateFormat: (SITE_SETTINGS as any).general?.defaultDateFormat ?? 'DD/MM/YYYY',
        theme: (typeof window !== 'undefined' ? localStorage.getItem('theme') as 'light' | 'dark' : null) || (SITE_SETTINGS as any).general?.theme || 'light',
        email: (SITE_SETTINGS as any).contact?.email ?? 'sangvish21@gmail.com',
        phone: (SITE_SETTINGS as any).contact?.phone ?? '+1 555-0000',
        copyright: (SITE_SETTINGS as any).contact?.copyright ?? '© 2026 CarRental. All rights reserved.',
        ...
        mapProvider: (SITE_SETTINGS as any).general?.mapProvider ?? 'osm',
        googleMapsApiKey: (SITE_SETTINGS as any).general?.googleMapsApiKey ?? '',
        ...
        ctaImageRenter: (SITE_SETTINGS as any).frontend?.ctaImageRenter ?? '/hero-car.png',
        ctaImageHost: (SITE_SETTINGS as any).frontend?.ctaImageHost ?? '/suv-car.png',
        ...
        enhanceImage: (SITE_SETTINGS as any).frontend?.enhanceImage ?? '/hero-car-new.png',
        appImage: (SITE_SETTINGS as any).frontend?.appImage ?? '/hero-main.jpg',
        ...
        officeLocations: (SITE_SETTINGS as any).locations?.officeLocations ?? [],
    });
    ...
    useEffect(() => {
        ...
        const fetchSettings = async () => {
            try {
                ...
                console.error(`Settings fetch failed (Attempt ${retryCount + 1}):`, error);
                if (retryCount < maxRetries) {
                    retryCount++;
                    setTimeout(fetchSettings, 2000); // Retry after 2 seconds
                } else {
    ...
    const contextValue = React.useMemo(() => ({ settings, updateSettings, loading }), [settings, loading]);
