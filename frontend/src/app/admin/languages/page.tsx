...
export default function AdminLanguages() {
 ...
 <Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="e.g. English" className="h-12 rounded-app bg-slate-50 border-none font-bold px-6 focus-visible:ring-primary/10" required />
 </div>
 <div>
 ...
 <Input value={formData.nativeName} onChange={e => setFormData({...formData, nativeName: e.target.value})} placeholder="e.g. English, Français" className="h-12 rounded-app bg-slate-50 border-none font-bold px-6 focus-visible:ring-primary/10" required />
 </div>
 <div>
 ...
 <Input value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} placeholder="en" className="h-12 rounded-app bg-slate-50 border-none font-bold px-6 focus-visible:ring-primary/10" required />
 </div>
 <Button type="submit" className="w-full h-14 bg-primary text-white font-black rounded-app mt-6 hover:bg-primary-hover transition-all">{t.languages.save}</Button>
 </form>
 </div>
 </div>
 )}
 ...
 {/* Header */}
 <div className="flex justify-between items-center bg-white p-6 rounded-app border border-slate-100 font-sans">
 <div>
 ...
 <Button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-primary-hover text-white h-11 px-6 rounded-app font-bold gap-2">
 <Plus size={18} /> {t.languages.add}
 </Button>
 </div>

 <div className="bg-white rounded-app border border-slate-100 overflow-hidden font-sans">
 ...
 <div className="w-8 h-8 rounded-app bg-primary/5 flex items-center justify-center text-primary border border-primary/10">
 ...
 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100 px-2.5 py-1.5 rounded-app bg-slate-50">{lang.code}</span>
 </td>
 <td className="px-6 py-4 text-center">
 ...
 <Button onClick={() => handleDelete(lang._id)} size="icon" variant="ghost" className="w-8 h-8 rounded-app text-slate-300 hover:text-rose-500 transition-colors">
 <Trash2 size={16} />
 </Button>
 </div>
 </td>
