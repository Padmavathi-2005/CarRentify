...
export default function AdminCurrencies() {
 ...
 setFormData({ name: "", code: "", symbol: "", exchangeRate: 1, symbolPosition: "left" });
 fetchCurrencies();
 }
 } catch (err) { console.error(err); }
 };
 ...
 <Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="e.g. US Dollar" className="h-12 rounded-app bg-slate-50 border-none font-bold px-6" required />
 </div>
 ...
 <Input type="number" step="0.0001" value={(formData as any).exchangeRate || ""} onChange={e => setFormData({...formData, exchangeRate: parseFloat(e.target.value)} as any)} placeholder="1.0" className="h-12 rounded-app bg-slate-50 border-none font-bold px-6" required />
 </div>
 </div>
 <div>
 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block px-1">Symbol Position</label>
 <select 
 value={formData.symbolPosition} 
 onChange={e => setFormData({...formData, symbolPosition: e.target.value})}
 className="w-full h-12 rounded-app bg-slate-50 border-none font-bold px-6 text-sm outline-none"
 >
 <option value="left">Left ($100)</option>
 <option value="right">Right (100$)</option>
 </select>
 </div>
 <Button type="submit" className="w-full h-14 bg-primary text-white font-black rounded-app mt-6 hover:bg-primary-hover transition-all">Save Currency</Button>
 </form>
 </div>
 </div>
 )}
 ...
 {/* Header */}
 <div className="flex justify-between items-center bg-white p-4 rounded-app border border-slate-100">
 <div>
 ...
 <Button onClick={() => { setIsModalOpen(true); setFormData({ name: "", code: "", symbol: "", exchangeRate: 1, symbolPosition: "left" }); }} className="bg-primary hover:bg-primary-hover text-white h-11 px-6 rounded-app font-bold gap-2">
 ...
 <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">{t.currencies.table.unit}</th>
 <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">{t.currencies.table.code}</th>
 <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Exchange Rate</th>
 <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">{t.currencies.table.symbol}</th>
 <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">{t.header.active || "Actions"}</th>
 </tr>
 </thead>
 <tbody>
 {loading ? (
 ...
 <div className="w-8 h-8 rounded-app bg-emerald-50 flex items-center justify-center text-emerald-600 border border-emerald-100">
 <Coins size={14} />
 </div>
 ...
 <span className="text-[10px] font-black text-primary bg-primary/5 px-2 py-1 rounded-app">{curr.code}</span>
 </td>
 <td className="px-6 py-3 text-center">
 <span className="text-[10px] font-bold text-slate-500">{curr.exchangeRate || 1}</span>
 </td>
 <td className="px-6 py-3 text-center font-black text-slate-900 text-sm">
 {curr.symbol}
 </td>
