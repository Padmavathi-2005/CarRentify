...
export class NewsletterService {
  ...
  async subscribe(email: string): Promise<{ message: string }> {
    ...
    if (!valid) {
      ...
      if (reason === 'typo')
        errorMsg = `Typo detected. Did you mean ${validators.typo?.reason}?`;
      else if (reason === 'disposable')
        errorMsg = 'Disposable emails are blocked.';
      else if (reason === 'smtp')
        errorMsg = 'This email inbox does not seem to exist.';
      else if (reason === 'mx')
        errorMsg = 'Invalid domain configured for email.';
